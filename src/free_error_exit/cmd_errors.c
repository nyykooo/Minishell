...
/*   Updated: 2024/08/07 12:54:25 by brunhenr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_execute_cmd(t_cmd *command, char **arguments, char **env_var)
{
	...
	{
		ft_print_error(command->shell, false, 127, 3, \
...
static void	ft_verify_path(t_cmd *command)
{
	if (command->path == NULL)
		ft_print_error(command->shell, true, 127, 3, \
		"-minishell: ", command->cmd, ": command not found\n");
}
...
static void	ft_analyze_cmd_stat(t_cmd *command)
{
	...
	if (stat(command->cmd, &buf) == 0)
	{
		...
		(ft_strncmp(command->cmd, "./", 2) == 0 || command->cmd[0] == '/'))
			ft_print_error(command->shell, true, 126, 3, \
